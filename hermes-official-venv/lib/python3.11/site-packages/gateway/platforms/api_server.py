...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Agent instance cache: reuse AIAgent across requests with the same
        # session_id to avoid ~2s re-initialization overhead per request.
        # Key: session_id, Value: (agent, created_at)
        import threading as _cache_threading
        from collections import OrderedDict as _OrderedDict
        self._agent_instance_cache: "_OrderedDict[str, tuple]" = _OrderedDict()
        self._agent_instance_cache_lock = _cache_threading.Lock()
        self._agent_instance_cache_max = int(os.getenv("HERMES_API_AGENT_CACHE_MAX", "4"))
        self._agent_instance_cache_ttl = int(os.getenv("HERMES_API_AGENT_CACHE_TTL", "300"))  # seconds
    ...
    def _create_agent(
        ...
        Agent instance caching: if session_id is provided and a cached agent
        exists for that session, reuse it instead of creating a new one.
        This eliminates ~2s of Agent initialization overhead per request.
        """
        # Try to reuse a cached agent for this session
        if session_id:
            with self._agent_instance_cache_lock:
                cached = self._agent_instance_cache.get(session_id)
                if cached is not None:
                    agent, created_at = cached
                    import time as _time
                    if _time.time() - created_at < self._agent_instance_cache_ttl:
                        # Move to end (most recently used)
                        self._agent_instance_cache.move_to_end(session_id)
                        ...
                        # TTL expired, evict
                        self._agent_instance_cache.pop(session_id, None)
        ...
        # Cache the agent instance for reuse on subsequent requests
        if session_id:
            with self._agent_instance_cache_lock:
                import time as _time
                self._agent_instance_cache[session_id] = (agent, _time.time())
                self._agent_instance_cache.move_to_end(session_id)
                # Evict oldest entries if cache is full
                while len(self._agent_instance_cache) > self._agent_instance_cache_max:
                    self._agent_instance_cache.popitem(last=False)
