#!/usr/bin/env python3
"""Test full Bridge dispatch for 4-category routing"""
...
for name, rtype, prompt, constraints, expect in tests:
    ...
    try:
        ...
        print(f"  [{mark}] {name}: route={actual_route}, status={status}, model={model}, strategy={strategy}")
        if note:
