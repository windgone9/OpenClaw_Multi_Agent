# OpenClaw 多智能体模型调度系统 — 技术说明书

> **版本**: 2026.4.27  
> **日期**: 2026-05-27  
> **说明**: 本文档基于代码实际实现编写，覆盖整体架构、各分模块详细设计、调用关系、上下游输入输出接口及实现技术细节
...
├── TECHNICAL_SPEC.md               # 原始技术规格书
└── SPECIFICATION.md                # 原始功能规格书
```
...
- 若 content 为空但存在 `reasoning_content`，则使用 reasoning_content 作为 output
- 计算 cost = (input_tokens/1000 × input_price) + (output_tokens/1000 × output_price)
- 设置 `routed_via_gateway = False`，`actual_model = None`（直连模式标识）

#### 3.5.2 LocalModelAgent

**文件**: `scheduler/agents/local_agent.py`

本地模型代理，`can_handle` 在有本地端点可用时返回 True。
...
6. 记录自适应结果 (strategy_router.record_adaptive_result)
7. fallback_results = None  (成功时不触发降级)
8. 执行 Post-Hook 管道
9. 构造 DispatchResponse 返回
```
...
| selected_endpoint | Optional[ModelEndpoint] | None | 选中的端点 |
| fallback_endpoints | List[ModelEndpoint] | [] | 降级端点列表 |
| strategy_name | str | "unknown" | 策略名称 |
| reason | str | "" | 路由原因 |
| use_openclaw | bool | False | 是否使用 OpenClaw |
...
│   ├── TOOL_CALL → 工具支持端点
│   ├── 长上下文 → 大上下文端点
│   ├── 特殊类型(EMBEDDING/IMAGE/AUDIO) → 专用端点
│   ├── 本地可处理 → 本地
│   └── 需要云端 → 云端
...
**辅助方法**：
- `record_adaptive_result(model_type, success, latency_ms)` → 记录结果到 AdaptiveStrategy
- `get_adaptive_state()` → 返回自适应权重和状态
...
#### 本地能力判断（`_can_local_handle`）

```
1. 若请求类型 ∈ {tool_call, image, audio}:
   - 检查本地端点是否 supports_tools
   - 无工具支持 → False
2. 检查本地端点 max_context_length ≥ 估计上下文长度
   - 不满足 → False
3. 默认 → True
```
...
| 算法 | 常量 | 说明 | 时间复杂度 |
|------|------|------|-----------|
| 轮询 | ROUND_ROBIN | 按顺序循环选择 | O(1) |
| 加权随机 | WEIGHTED_RANDOM | 按 weight 概率选择 | O(n) |
| 最少连接 | LEAST_CONNECTIONS | 选 current_load 最小的 | O(n) |
| 最低延迟 | LEAST_LATENCY | 选 avg_latency_ms 最小的 | O(n) |
| Power-of-Two | POWER_OF_TWO | 随机选2个，取负载低的 | O(1) |
...
ollama-qwen2.5（priority=1, load_factor=0, success_rate=1.0, 无约束）：
```
score = 100.0 - 0 - 0 + 10.0 = 110.0
```

kimi-k2.6（priority=2, load_factor=0.2, success_rate=0.98, preferred_providers=["moonshot"]）：
```
score = 100.0 - 15.0 - 4.0 + 9.8 + 20.0 = 110.8
```
...
- `priority > 50` 的 Hook 失败仅记录日志，不中断
...
| ID | Provider | 模型ID | 类型 | 工具支持 | 上下文长度 | 输入成本/1k |
|----|----------|--------|------|---------|-----------|------------|
| ollama/qwen2.5:3b | ollama | qwen2.5:3b | local | 否 | 32768 | $0.00 |
| moonshot/kimi-k2.6 | moonshot | kimi-k2.6 | cloud | 是 | 262144 | $0.76 |
| deepseek/deepseek-chat | deepseek | deepseek-chat | cloud | 否 | 64000 | $0.00014 |
...
{
  currentLoad: 0,        // 当前负载
  maxConcurrent: 5/20,   // 最大并发（ollama=5, 云端=20）
  avgLatencyMs: 0,       // 平均延迟
  successRate: 1.0,      // 成功率
...
#### 配置加载（loadOpenClawConfig）

从项目根目录的 `openclaw.json` 读取配置，提供：
- Agent 列表及模型配置
- 路由策略配置
- Gateway 配置
...
- 提取 `choices[0].message.content` 作为输出
- 计算 cost = (input_tokens/1000 × input_price) + (output_tokens/1000 × output_price)
- `routed_via_gateway` 标记是否经过 Gateway
- `actual_model` 记录 Gateway 返回的实际模型ID（`data.model`）
...
单例模式（`Settings.get_instance()`），从环境变量读取配置：
...
│                    MainDispatcherAgent                            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐  │
│  │ HookManager   │  │StrategyRouter│  │   Agent Layer        │  │
│  │              │  │              │  │                      │  │
│  │ Pre-Hooks:   │  │ Adaptive     │  │ ┌────────────────┐  │  │
│  │  rate_limit  │  │ Strategy     │  │ │ LocalModelAgent│  │  │
│  │  validation  │  │              │  │ └────────────────┘  │  │
│  │  enrichment  │  │ Priority     │  │ ┌────────────────┐  │  │
│  │  logging     │  │ Strategy     │  │ │ CloudModelAgent│  │  │
│  │              │  │              │  │ └────────────────┘  │  │
│  │ Post-Hooks:  │  │ LoadBalance  │  │ ┌────────────────┐  │  │
│  │  sanitization│  │ Strategy     │  │ │ OpenClawAgent  │  │  │
│  │  cost_calc   │  │              │  │ └───────┬────────┘  │  │
│  │  retry       │  └──────┬───────┘  └─────────┼───────────┘  │
│  │  logging     │         │                     │              │
│  └──────────────┘         │                     │              │
