...
echo "[2/2] 在 proxy pod 内注册模型 (经 litellm:4000 桥接, stdin 管道传脚本)..."
# kubectl exec -i 解析 deployment; python3 - 从 stdin 读脚本; env 内联传 key ($var 本地展开)
# proxy pod 经 litellm:4000 (14-bridge) 直达现有 litellm
kubectl -n openclaw exec -i deploy/proxy -- sh -c "VLLM_API_KEY=${VLLM_API_KEY} LITELLM_MASTER_KEY=${LITELLM_MASTER_KEY} python3 - --base http://litellm:4000" < scripts/litellm_configure.py
